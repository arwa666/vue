  ...
  var ProgressSteps = function(el, options) {
    ...
    if (this.$stepItems.length !== this.$stepContent.length) {
      console.error(
        "Progress Steps - You must provide a [data-step] for each [data-target-step] in your markup"
      );
      return;
    }
    // if (!this.$stepForwardBtn.length) {
    //   console.error(
    //     'Progress Steps - You must provide a "next step" button using [data-step-forward] in your markup'
    //   );
    //   return;
    // }
    // if (!this.$stepBackBtn.length) {
    //   console.error(
    //     'Progress Steps - You must provide a "previous step" button using [data-step-back] in your markup'
    //   );
    //   return;
    // }
  ...
  ProgressSteps.prototype.gotoStep = function(step) {
    ...
    var targetStep = this.$stepItems.filter(
    ...
    // step === 1
    //   ? this.$stepBackBtn.attr("disabled", true)
    //   : this.$stepBackBtn.attr("disabled", false);
    // step === this._numSteps
    //   ? this.$stepForwardBtn.attr("disabled", true)
    //   : this.$stepForwardBtn.attr("disabled", false);
  };
  ...
  $.fn.progressSteps = function(option) {
    return this.each(function() {
      ...
      if (!data)
        $this.data("bs.progressSteps", new ProgressSteps(this, options));
    });
  };
